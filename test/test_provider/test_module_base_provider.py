...
def test_locates_module_in_namespace_package(with_fake_python_namespace_module):
    with_fake_python_namespace_module('/archist', 'npkg1.nsub2', 'a_module')
    module_nodes = BasicModuleProvider().provide_from('/archist')
    assert len(module_nodes) == 1


def test_module_from_namespace_package_has_correct_location(

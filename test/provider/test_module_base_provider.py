...
def test_locates_modules_in_package(with_fake_python_module):
    with_fake_python_module('/archist', 'pkg1.sub2', 'a_module')
    module_nodes = BasicModuleProvider().provide_from('/archist')
    assert len(module_nodes) == 3


def test_module_has_correct_location(with_fake_python_module):
    with_fake_python_module('/archist', 'pkg1.sub2', 'a_module')
...
def test_locates_module_in_namespace_package(with_fake_python_ns_module):
    with_fake_python_ns_module('/archist', 'npkg1.nsub2', 'a_module')
    module_nodes = BasicModuleProvider().provide_from('/archist')
    assert len(module_nodes) == 1


def test_module_from_namespace_package_has_correct_location(with_fake_python_ns_module):
    with_fake_python_ns_module('/archist', 'npkg1.nsub2', 'a_module')
